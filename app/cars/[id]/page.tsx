...
import { CarBookingPanel } from "@/components/listings/BookingPanel";
...
export default async function CarDetailPage({ params }: Props) {
  ...
  const listing = LISTINGS.find((l) => l.id === id);
  if (!listing) notFound();
  const related = LISTINGS.filter((l) => l.type === "car" && l.id !== listing.id).slice(0, 4);
      ...
      <main className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="grid lg:grid-cols-[1fr_340px] gap-8">
              ...
              <div className="grid grid-rows-2 gap-2">
                {listing.images.slice(1, 3).map((img, i) => (
                  <div key={i} className="relative aspect-video rounded-xl overflow-hidden">
                    <Image src={img} alt={`${listing.title} view ${i + 2}`} fill className="object-cover" sizes="25vw" />
                  </div>
                ))}
              </div>
            </div>
            <h1 className="text-2xl font-bold text-[#0D1B2A] mb-1">{listing.title}</h1>
            <p className="text-sm text-[#6B7280] mb-4">{listing.color} • {listing.fuel} • {listing.transmission}</p>
          ...
          <div className="lg:sticky lg:top-20 space-y-4">
            <CarBookingPanel listing={{ id: listing.id, title: listing.title, images: listing.images, pricePerDay: listing.pricePerDay, location: listing.location, type: "car" }} />
