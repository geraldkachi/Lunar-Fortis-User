...
export default function CartPage() {
  const { isLoggedIn, cart, removeFromCart } = useAuth();
  ...
  return (
    <>
      ...
      <main className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-10 min-h-[70vh]">
        <h1 className="text-2xl font-bold text-[#0D1B2A] mb-8">Your Cart</h1>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button onClick={() => removeFromCart(item.id)}
                          className="text-[#9CA3AF] hover:text-[#E8392A] transition-colors flex-shrink-0">
                          <Trash2 size={16} />
                        </button>
                      </div>
                      <div className="mt-2 text-xs text-[#6B7280] space-y-0.5">
                        {item.type === "accommodation" && (
                          <>
                            ...
                            <p>Guests: <span className="font-medium text-[#0D1B2A]">{item.guests}</span></p>
                        ...
                        {item.type === "car" && (
                          <>
                            ...
                            <p>Preference: <span className="font-medium text-[#0D1B2A]">{item.preference}</span></p>
                            <p>{nights} day{nights > 1 ? "s" : ""}</p>
                          </>
                        )}
                      </div>
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-xs text-[#6B7280]">{formatPrice(item.pricePerDay)}/day</span>
              ...
              <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 sticky top-20">
                ...
                {cart.map((item) => (
                  <Link key={item.id} href={`/payment/${item.id}`}
                    className="flex items-center justify-center gap-2 w-full bg-[#0D1B2A] hover:bg-[#1a2d40] text-white font-semibold py-3.5 rounded-xl transition-colors text-sm mb-2">
                    Pay {formatPrice(item.pricePerDay * (item.nights ?? item.days ?? 1))}
                    <ArrowRight size={15} />
                  </Link>
                ))}
                <p className="text-xs text-center text-[#6B7280] mt-2">Secure payment powered by Flutterwave</p>
              </div>
            </div>
          </div>
        )}
